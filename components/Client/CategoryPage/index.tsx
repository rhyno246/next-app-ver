...
import SingleGridItem from "../ShopPage/SingleGridItem";
import SingleListItem from "../ShopPage/SingleListItem";
...
export default function CategoryPage({ slug }: { slug: string }) {
  ...
  const { data, loading } = useQuery<CategoryPageResponse>(CATEGORY_BY_SLUG, {
    variables: { slug },
  });
  if (loading) {
  ...
  return (
    <>
      <Breadcrumb title={"Category"} pages={[slug]} />
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            ...
            <div className="w-full">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <p>
                      ...
                      <span className="text-dark">
                        {data?.categoryBySlug.products.length} Products
                      </span>
                    </p>
                  </div>
              ...
              {(data?.categoryBySlug.products.length ?? 0) > 0 ? (
                <div
                  className={`${
                    productStyle === "grid"
                ...
                >
                  {data?.categoryBySlug.products.map((item, key) =>
                    productStyle === "grid" ? (
                      <SingleGridItem item={item.product} key={key} />
                    ) : (
                      <SingleListItem item={item.product} key={key} />
                    ),
                  )}
                </div>
              ) : (
                <p className="text-center text-dark text-lg">
