...
const SingleItem = ({ testimonial }: { testimonial: Comments }) => {
  return (
    <div className="shadow-testimonial bg-white rounded-[10px] py-7.5 px-4 sm:px-8.5 m-1">
      <div className="flex items-center gap-1 mb-5">
        {[1, 2, 3, 4, 5].map((star) => (
          <svg
            key={star}
            className={`fill-current ${star <= testimonial.rating ? "text-[#FFA645]" : "text-gray-3"}`}
            width="15"
            height="15"
            viewBox="0 0 18 18"
            fill="none"
      ...
      <div className="mt-4">{testimonial.content}</div>
    </div>
  );
};
