...
import { GET_ME, GET_SITE_CONFIG_MULTI } from "@/graphql/queries";
import { ConfigResponse, MeResponse } from "@/types/type";
...
export default function Header() {
  ...
  const { data: settings } = useQuery<ConfigResponse>(GET_SITE_CONFIG_MULTI);
  const [stickyMenu, setStickyMenu] = useState(false);
  const [option, setOPtion] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [navigationOpen, setNavigationOpen] = useState(false);
  const [logout, { error }] = useMutation(LOGOUT_AUTHOR);
            ...
            <div className="max-w-118.75 w-full">
              <form onSubmit={handleSubmitSearch}>
                <div className="flex items-center">
                  ...
                  <div className="relative max-w-83.25 sm:min-w-83.25 w-full">
                ...
                {user ? (
                  <div className="relative group">
                    <div className="rounded-full overflow-hidden relative  w-8 h-8">
                      <Image
                        src={
                          user.image ? user.image : "/images/users/user-04.jpg"
                        }
                    ...
                    <ul
                      className={`
                        absolute left-0 mt-2 px-5 bg-white shadow-lg rounded-md p-3 
                        transition-all duration-200 min-w-50

                        ${option ? "opacity-100 visible" : "opacity-0 invisible"}
                    ...
                    >
                      ...
                      {user.role.name === roleType.type_admin && (
                        <li className="block py-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <div
              className={`w-[288px] absolute right-4 top-full xl:static xl:w-auto h-0 xl:h-auto invisible xl:visible xl:flex items-center justify-between ${
                navigationOpen &&
                `visible! bg-white shadow-lg border border-gray-3 h-auto! max-h-100 overflow-y-scroll rounded-md p-5`
              }`}
            >
              {/* <!-- Main Nav Start --> */}
              <nav>
                <ul className="flex xl:items-center flex-col xl:flex-row gap-4 xl:gap-10">
                  <CustomLink
                    item={{ label: "Home", href: "/" }}
                    stickyMenu={stickyMenu}
                  />
                  <CustomLink
                    item={{ label: "Shop", href: "/shop" }}
                    stickyMenu={stickyMenu}
                  />
                  <CustomLink
