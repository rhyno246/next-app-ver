...
const SingleItem = ({ item }: Props) => {
  const salePrice =
    item.sale > 0 ? Math.round(item.price * (1 - item.sale / 100)) : item.price;
  const { removeItemCart, increaseQuantityItem, decrementQuantityItem } =
    useCartStore();
  ...
  return (
    <div className="flex items-center border-t border-gray-3 py-5 px-7.5">
      ...
      <div className="min-w-[180px]">
        <p className="text-dark">${salePrice}</p>
      </div>
      ...
      <div className="min-w-[200px]">
        <p className="text-dark">${salePrice * item.quantity}</p>
      </div>
